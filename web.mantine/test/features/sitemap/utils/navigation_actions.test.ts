...
import { halClient } from '../../../../src/lib/hal_forms_client';
import { ApiClientError } from '../../../../src/services/api.client';
import { notifications } from '@mantine/notifications';

// Mock dependencies
vi.mock('../../../../src/lib/hal_forms_client', () => ({
    halClient: {
        post: vi.fn(),
    },
}));
...
describe('executePostAction', () => {
    ...
    it('should execute POST request and navigate to self link on success', async () => {
        ...
        vi.mocked(halClient.post).mockResolvedValue(mockResponse);

        await executePostAction('/users/abc123/api-keys/list', 'API Keys', mockNavigate);

        // Verify POST request made
        expect(halClient.post).toHaveBeenCalledWith('/users/abc123/api-keys/list');
    ...
    it('should show error notification on 401 unauthorized', async () => {
        ...
        vi.mocked(halClient.post).mockRejectedValue(error);
    ...
    it('should force full page reload when response includes federate link', async () => {
        ...
        expect(halClient.post).toHaveBeenCalledWith('/auth/revoke');
