import type { HalLink, HalTemplate, HalObject } from '@houseofwolves/serverlesslaunchpad.types/hal';
import type { NavItem, NavGroup, ResolvedNavItem } from '../types';
...
export interface UseNavigationResult {
  ...
  itemExists: (item: NavItem) => boolean;
  isNavItem: (item: NavItem | NavGroup) => item is NavItem;
  isNavGroup: (item: NavItem | NavGroup) => item is NavGroup;
  loading: boolean;
  error: string | null;
  refetch: () => Promise<void>;
}

export interface UseNavigationDependencies {
  data: HalObject | null;
  loading: boolean;
  error: string | null;
  refetch: () => Promise<void>;
 ...
 *
 * Provides utilities to resolve navigation items to their actual links or templates.
 * This enables building menus from the HATEOAS-defined navigation structure.
 *
 * @param deps - Dependencies injected by the consumer (data fetching hook)
 * @returns Navigation data and resolver functions
 *
 * @example
 * ```typescript
 * // In a React project, wrap this with a project-specific hook:
 * function useNavigation() {
 *   const deps = useHalResource('/sitemap');
 ...
 * ```
 */
export function useNavigation(deps: UseNavigationDependencies): UseNavigationResult {
  const { data, loading, error, refetch } = deps;
